...
const ChoiceButtons = React.memo(function ChoiceButtons({
  choices,
  onSelect,
  disabled = false,
}: ChoiceButtonsProps) {
  ...
  return (
    <div
      className="flex flex-col gap-3 w-full max-w-lg min-h-[120px]"
      role="group"
      ...
      {choices.map((choice, i) => (
        <motion.button
          ...
          className={`w-full px-6 py-3.5 rounded-xl border text-right transition-all duration-200 text-base leading-relaxed font-bold min-h-[44px] ${
            choice.isComedy
              ? 'bg-gradient-to-b from-[rgba(168,85,247,0.18)] to-[rgba(168,85,247,0.06)] border-[rgba(168,85,247,0.5)] text-purple-300 shadow-[0_4px_12px_rgba(0,0,0,0.15),0_1px_3px_rgba(0,0,0,0.2)] hover:shadow-[0_0_20px_rgba(168,85,247,0.15),0_4px_12px_rgba(0,0,0,0.2)] active:scale-[0.96] active:shadow-[inset_0_2px_8px_rgba(0,0,0,0.4),inset_0_0_20px_rgba(168,85,247,0.2)]'
        ...
        >
          ...
          {choice.isComedy && <span className="mr-2 text-sm opacity-60">😂</span>}
        </motion.button>
      ))}
    </div>
  );
